...
| **Technical Analysis** | `chart-shots/` mtime + metadata (symbol/TF/pattern/bias/invalidation). **DARK until shot+as-of** |
...
| Technical Analysis | `chart-shots/` readable; show latest shot as-of | Path missing / no metadata / no as-of → DARK |
...
CX notes: Chart shots tab (TA; files under chart-shots/; DARK until shot+as-of)
