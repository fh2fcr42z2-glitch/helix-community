...
6. **Fine-tune** — fold lessons into a **hypothesis graph** (what failed, where, in which regime). Negative results are first-class. Next cycle must not restart from ignorance. For Chart bot / TA this is **file-based**: a `ta-learning/` retro + PATTERN-EDGE-LOG row — **not silent weight magic**. Viral PnL is never a training label.
...
- Stage 6 stores failures in warehouse / memory with `quality_flag` — never rewrite history. Chart bot stores them as retros in [`ta-learning/`](../ta-learning/), not hidden weights.
